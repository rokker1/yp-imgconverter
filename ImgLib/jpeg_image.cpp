...
// функция 
void LoadFromImageToBuffer(JSAMPLE* row, int y, const Image& in_image) {
    const Color* line = in_image.GetLine(y);
    for (int x = 0; x < in_image.GetWidth(); ++x) {
        row[3 * x + 0] = static_cast<unsigned char>(line[x].r);
        row[3 * x + 1] = static_cast<unsigned char>(line[x].g);
        row[3 * x + 2] = static_cast<unsigned char>(line[x].b);
    }
}
...
bool SaveJPEG(const Path& file, const Image& image) {
    ...
    */
     ...
     */
        ...
        LoadFromImageToBuffer(&buffer[0], y, image);
